...
<template>
  <div :class="['flex min-h-screen relative transition-colors duration-300',
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none z-0">
      <div class="absolute -top-32 -right-32 w-[450px] h-[450px] rounded-full opacity-20"
           :class="theme.isDark ? 'bg-bordeaux-700' : 'bg-bordeaux-400'"
           style="filter: blur(100px)"></div>
      <div class="absolute bottom-0 left-1/4 w-80 h-80 rounded-full opacity-15"
           :class="theme.isDark ? 'bg-bordeaux-900' : 'bg-bordeaux-300'"
           style="filter: blur(80px)"></div>
    </div>

    <!-- Sidebar -->
    <aside :class="['fixed left-0 top-0 bottom-0 z-50 flex flex-col backdrop-blur-xl border-r transition-all duration-300',
      sidebarOpen ? 'w-60' : 'w-16',
      theme.isDark ? 'bg-black/30 border-white/10' : 'bg-white/20 border-white/40']">

      <!-- Header -->
      <div :class="['flex items-center p-3 border-b',
        theme.isDark ? 'border-white/10' : 'border-bordeaux-200/50']">
        ...
        <RouterLink
          ...
          :class="['flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all no-underline',
            theme.isDark
              ...
              : 'text-bordeaux-700 hover:bg-bordeaux-700/10 hover:text-bordeaux-900',
            !sidebarOpen && 'justify-center']"
          activeClass="!bg-bordeaux-700/20 !text-bordeaux-700 dark:!text-bordeaux-300 font-semibold"
        >
    ...
    <div :class="['flex-1 flex flex-col relative z-10 transition-all duration-300',
      ...
      <!-- Topbar -->
      <header :class="['sticky top-0 z-40 flex items-center justify-between px-6 py-3 backdrop-blur-xl border-b',
        theme.isDark ? 'bg-black/20 border-white/10' : 'bg-white/20 border-white/40']">
        <h2 :class="['text-base font-semibold', theme.isDark ? 'text-bordeaux-200' : 'text-bordeaux-800']">
          {{ route.meta.title ?? 'Tableau de bord' }}
        </h2>
        <div class="flex items-center gap-2">
          <div :class="['w-8 h-8 rounded-full flex items-center justify-center',
            theme.isDark ? 'bg-bordeaux-800/60 text-bordeaux-300' : 'bg-bordeaux-200/60 text-bordeaux-700']">
